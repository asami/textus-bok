...
| `nodes` | Array of source nodes. Each node has non-empty `id`, `label`, and `node_type`; `category`, `terms`, and `tags` are optional source metadata. A `component-reference` node may carry `componentRef` with required `kind` (`car` or `sar`) and `name`, plus optional non-empty `organization` and `version`. |
...
CAR/SAR entry in the same selected source generation's component index, and
any declared `organization` or `version` also exactly matches that entry. It
...
The 2026-07-23 representative Cozy BoK generated by Cozy `0.3.0-SNAPSHOT`
contained 95 nodes and 150 edges. The
