...
- [x] Focused executable specifications cover profile vocabulary, resolver
  isolation, read filtering, attribution, defaulting, Web forms, MCP schemas,
  authorization, and failure behavior.
- [x] One representative SAR registers official, development, and two distinct
  project-local generations and proves that no request observes records from a
  non-selected profile or project.
- [x] REST, Static Form Web, and MCP apply equivalent profile resolution and
  ...
  remains outside MCP.
- [x] The full Textus BoK test suite, `cozyBuildCAR`, normal CAR lint, and the
  representative selection checks pass with recorded evidence.
- [x] README, strategy, stable design/specification, developer/operator guides,
